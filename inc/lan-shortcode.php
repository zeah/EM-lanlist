...
// wp_die('<xmp>'.print_r(get_post_types(['public' => true]), true).'</xmp>');
	...
	public function add_shortcode_bilde($atts, $content = null) {
		...
		// 	if (preg_match('/(?:(?!\?|&))(?:source=.*?)(?:(&|$))/', $meta['pixel'], $matches))
		// 		$meta['pixel'] = str_replace($matches[0], '', $meta['pixel']); 

		// 	$meta['pixel'] = preg_replace('/\?$/', '', $meta['pixel']);
 
		// 	if (strpos($meta['pixel'], '?') !== false) $meta['pixel'] .= '&source=' . $atts['source'];
		// 	else $meta['pixel'] .= '?source=' . $atts['source'];
		// }
		
		// if ($meta['pixel']) $this->add_pixel($meta['pixel']);
		// image with anchor
